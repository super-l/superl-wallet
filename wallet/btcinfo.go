...
func (w *Wallet) BtcGetUncompressedPubKey() []byte {
	...
	pubkeyUncompressedStr := fmt.Sprintf("%s%x%x", "04", w.PrivateKey.X.Bytes(), w.PrivateKey.Y.Bytes())
...
func (w *Wallet) BtcGetCompressedPubKey() []byte {
	...
	result := data.Mod(w.PrivateKey.Y, big.NewInt(2))
	...
	if result.Cmp(big.NewInt(0)) == 0 {
		//fmt.Print("偶数\n")
		pubkeyCompressedStr = fmt.Sprintf("%s%x", "02", w.PrivateKey.X.Bytes())
	} else {
		//fmt.Print("奇数\n")
		pubkeyCompressedStr = fmt.Sprintf("%s%x", "03", w.PrivateKey.X.Bytes())
	}
